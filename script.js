...
const getData = async (query, value = "", page) => {
  //fetch function
  try {
    let url = `${BASE_URL}`;
    if (query === "features") {
      ...
      url = `${BASE_URL}/games?${query}=${value}&page=${page}`;
    }
...
const renderGame = async (query, value, page) => {
  display.innerHTML = `<div class="infinity-1"></div>`;

  const res = await getData(query, value, page);
  display.innerHTML = "";
...
const getSpecificGame = async (value) => {
  ...
  try {
    url = `${BASE_URL}/single-game/${value}`;
    const res = await fetch(url);
    const data1 = await res.json();
    console.log(data1);
    return data1;
...
nextPageBtn.addEventListener("click", () => {
  ...
  renderGame(
    "genres",
    `${categoryGroup.children[2].textContent}`,
    `${counter}`
  );
});
